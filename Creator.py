...
TEMPMAIL_BASE_URL = "https://tempmail4me.eu"
TEMPMAIL_COOKIES = {
    'tm_mails': '',
    'XSRF-TOKEN': 'eyJpdiI6ImI3UWFZVHRzU2ZGcjNWY0xBRUtkcnc9PSIsInZhbHVlIjoiY1M1ZkV0V2dzYXpyV1d0MlFlVzVJd0s1eWs4K1NhTnh2TVl3ejNvY2tEK2EyY25EYzRyUm9ER2tVeXlrK2JzOSIsIm1hYyI6ImIxYWQ4YzM1MjNjYjNhMTc1MDQ3NWFmZmQ0NTRlNTI1MzM4ZTE0YWNkNjExM2MwYzBmNzhmMDI2MzM0OWZiYWYifQ%3D%3D',
    'temp_mail_4_me_session': 'eyJpdiI6IjFRbkVLTVNwV05CclFsNkY5dzNSTHc9PSIsInZhbHVlIjoiZXF0QjY1VThKZGpUNGtUY1wvUGZZeU5lRlVWUWdzZEhtUnloSFwvcGNlOEdQNm4wMXg2XC93MXFjU0dxWjFSQU5rRSIsIm1hYyI6ImU3MjYyYmFkNzc0NWM2MjU1Mjk0M2ViZjVkZWVhYmJmMTRkNTU3YTA2NGY1YTY1MjdiYzAyZDAwMDIyNzRiNzYifQ%3D%3D'
}

def get_fixed_ua():
    return "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Mobile Safari/537.36"

def generate_tempmail():
    """Generate a random email address for tempmail"""
    username = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
    return f"{username}@myglobalmail.eu"

def fetch_otp_from_temp4me(email, retries=2, delay=5):
    mailbox_url = f"{TEMPMAIL_BASE_URL}/mailbox/{email}"
    fetch_url = f"{TEMPMAIL_BASE_URL}/mail/fetch?new=true"
    headers = {
        "User-Agent": get_fixed_ua(),
        "Accept": "application/json",
        "Referer": mailbox_url,
        "X-Requested-With": "XMLHttpRequest"
    }
    requests.get(mailbox_url, cookies=TEMPMAIL_COOKIES, headers=headers)
    time.sleep(2)
    for _ in range(retries):
        try:
            resp = requests.get(fetch_url, headers=headers, cookies=TEMPMAIL_COOKIES)
...
logo=("""
   ...
   [green1]╚══════╝╚═╝░░╚═╝╚═╝░░╚═╝░╚════╝░╚═╝░░╚═╝    [green1][bold]VERSION/0.5
          ...
          [bold magenta2]CREATED BY ONE MAN CODE
""")
ll=str([hari,tanggal])
kk=str([xy,co])
    ...
    for make in range(int(num_accounts)):
        while True:  # Network retry loop
            try:
                ...
                email2 = generate_tempmail()
                phone2 = GetPhone()
                email3 = GetEmails()
                firstname,lastname = fake_name()
                print(Panel(f"[bold white] ACCESSING FACEBOOK",style="bold red"))
                dn()
                print(Panel(f"[bold white] REGISTERING ACCOUNT",style="bold red"))
                dn()
                print(Panel(f"[bold white] FILLING UP CREDENTIALS",style="bold red"))
                dn()
                print(Panel(f"[bold white] CHECKING ACCOUNT LIVE OR NOT",style="bold red"))
                dn()
                print(Panel(f"[bold white] GETTING MAIL & GETTING PHONE",style="bold red"))
                dn()
                print(Panel(f"[bold white] DUMMYPHONE : [bold green1]{phone2}",style="bold red"))
                dn()
                print(Panel(f"[bold white] DUMMYEMAIL : [bold green1]{email3}",style="bold red"))
                dn()
                print(Panel(f"[bold white] EMAIL : [bold green1]{email2}",style="bold red"))
                dn()
                print(Panel(f"[bold white] FIRSTNAME : {firstname}",style="bold red"))
                dn()
                print(Panel(f"[bold white] LASTNAME : {lastname}",style="bold red"))
                dn()
                print(Panel(f"[bold white] PASSWORD : {passw}",style="bold red"))
                dn()
                print(Panel(f"[bold white] FILLING CREDENTIALS [bold green1]DONE  ",style="bold red"))
                dn()
                print(Panel(f"[bold green1] ACCOUNT LIVE",style="bold red"))
                dn()
                print(Panel(f"[bold white] DATE & TIME : [bold green1]{tanggal} {waktu}",style="bold red"))
                dn()
                payload = {
                    'ccp': "2",
                ...
                if "c_user" in py_submit.cookies:
                    ...
                    valid = fetch_otp_from_temp4me(email2)
                    if valid:
                ...
                print(Panel(f"[bold red] NO NETWORK CONNECTION, WAITING FOR NETWORK...",style="bold magenta2"))
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            dn()
            with open("/sdcard/AUTO-BRYX/SUCCESS-OK-ID.txt","a") as f:
                f.write(uid + f"|{passw}|" + cookie + "\n")
